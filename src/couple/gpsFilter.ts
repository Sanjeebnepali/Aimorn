 ...
 * track" algorithm. Ported verbatim (pure math, no backend coupling) from
 * the source feature's lib/gpsFilter.ts.
 *
 ...
 *
 * WHY adaptive: a single fixed Q is a bad compromise. Deriving Q from the
 * (Doppler-based, far more reliable than differencing two noisy positions)
 * speed lets us smooth hard when still and lightly when moving:
 *
 *   still   (≈0 m/s)    → Q_MIN  → rock-steady number
 *   walking (~1.4 m/s)  → Q≈4.8  → tracks within ~2 fixes
 *   running (≥~2.5 m/s) → Q_MAX  → minimal lag
 *
 * Speed is unavailable on some devices/fixes (null, or −1 on iOS when invalid)
 * → fall back to the caller's default Q, so behaviour is never worse than a
 * fixed-Q filter.
 */
const Q_MIN = 0.6;
const Q_MAX = 8;
...
function qForSpeed(speedMps: number | null | undefined, qDefault: number): number {
 ...
 */
export function acceptFix(lat: number, lng: number, accuracyM: number | null, timeMs: number): boolean {
