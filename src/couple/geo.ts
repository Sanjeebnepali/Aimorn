 ...
 * buffer zone, and the great-circle helper. Extracted from `store.ts` so the
 * store file holds the state machine and this file holds the math — ported
 * near-verbatim from the source feature's store/couple.geo.ts, which was
 * already Supabase-free (pure math, no backend coupling).
 */
import type { ProximityState, State } from './types';
 ...
 */
export function correctedDistanceM(d: number, myAccuracy: number | null, partnerAccuracy: number | null): number {
  const a = Math.max(myAccuracy ?? MIN_FIX_UNCERTAINTY_M, MIN_FIX_UNCERTAINTY_M);
  const b = Math.max(partnerAccuracy ?? MIN_FIX_UNCERTAINTY_M, MIN_FIX_UNCERTAINTY_M);
 ...
 */
export function recomputeDistance(s: State, set: (patch: Partial<State>) => void): void {
  const { myLat, myLng, partnerLat, partnerLng, myAccuracy, partnerAccuracy, proximity, paused } = s;
  ...
  const accs = [myAccuracy, partnerAccuracy].filter((a): a is number => a != null && Number.isFinite(a));
  const { near, far } = getBufferZone(accs.length ? Math.max(...accs) : null, s.thresholdM);
 ...
 *
 * `thresholdM` (default 100 = the design baseline) scales the whole band
 * proportionally — `thresholdM/100` — so the couple's configured threshold
 * (Couple.thresholdM) is honoured instead of being dead state. Clamped to a
 * sane 0.2–5× range so a corrupt/extreme value can't collapse the band to
 * ~0 m or blow it up to kilometres.
 */
export function getBufferZone(accuracy: number | null, thresholdM: number = 100): { near: number; far: number } {
  let near: number;
  let far: number;
  if (accuracy == null) {
    near = 150;
