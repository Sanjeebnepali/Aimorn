...
// Moved from src/store/__tests__/couple.geo.test.ts (which tested the donor
// app's now-deleted store/couple.geo.ts) to test the real, live file this
// app actually uses. The math is identical — geo.ts's own doc comment notes
// it was "ported near-verbatim" — only the State shape differs (flat
// `hasPartner`/`partner`/`myRole`/`packId` here, not the donor's nested
// `link` object), so only baseState() below needed updating.
