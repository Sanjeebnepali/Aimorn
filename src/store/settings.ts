...
export type SettingsState = {
  ...
  // `resolution`, `liveWallpaper`, `generateQuality`, `newWallpaperAlerts`,
  // `dailyRecommendation` were removed here (audit, 2026-08-29) — same
  // reason as maxGenPerDay above: persisted fields with a Settings-screen
  // control, but zero call sites anywhere ever read them. Toggling them
  // did nothing; the "Daily Recommendation" row even promised a specific
  // time ("9:00 AM") that nothing scheduled. See the UI-side removal in
  // components/settings/PreferenceSections.tsx for the full writeup.
  // Notifications
  vibrationOnDownload: boolean;
  // Onboarding — has the user ever been shown the "allow always-on
  // background" prompt? Set true the first time it's shown (any background
  // feature — shuffle / mood / friend / sleep-wake — being enabled can
  // trigger it). Bookkeeping only: `lib/backgroundAccess.ts` does NOT gate
  // re-prompting on this flag, since the app isn't actually exempt until
  // the user follows through, and it re-offers every session until it is.
  bgAccessPrompted: boolean;
  // Same idea for the Android 12+ "Alarms & reminders" (exact-alarm) grant —
  // a SEPARATE prompt that gives to-the-minute timing, tracked apart from
